...
const Carousel: React.FC<CarouselProps> = ({ sliders, mainTitle, backGroundImage }) => {
        ...
        const filterSliders = async () => {
            const validSliders = await Promise.all(
                sliders.map(async (slider, index) => {
                    const response = await fetch(`/service/${index + 1}`);
                    if (response.ok) {
                        return slider;
                    } else {
                        return null;
            ...
            setFilteredSliders(validSliders.filter(slider => slider !== null) as Slider[]);
        };

        filterSliders();
    }, [sliders]);

    const handleRedirect = async (index: number) => {
        const response = await fetch(`/service/${index}`);
        if (response.ok) {
    ...
    return (
        <div className="overflow-hidden">
            <div className="relative flex pl-[16px] items-center md:pl-[160px]">
                ...
                ></div>
                <h2 className="text-[28px] pl-4 font-bold uppercase md:text-[50px] text-white">{mainTitle}</h2>
            </div>
            ...
            >
                ...
                >
                    {filteredSliders.map((slider, index) => (
                        <SwiperSlide
                            key={index}
                            className={`bg-transparent w-[100%] ${index === 0 ? 'swiper-slide-first' : ''} ${index === filteredSliders.length - 1 ? 'swiper-slide-last' : ''}`}
                        >
                            <div className="flex flex-col items-center justify-center text-center space-y-4 md:mt-[163px] md:flex-row md:items-center md:space-y-0 md:space-x-8">
                                <div className="relative mt-[50px] flex-shrink-0 md:w-[379px] md:h-[379px]">
                                    <img src={IMAGES_URL + slider.carousel_images.data[0].attributes.url} alt={`Icon ${index + 1}`} className="w-full h-full" />
                                    {slider.carousel_images.data[1] && (
                                        <div className="absolute inset-0 flex justify-center items-center">
                                            <img src={IMAGES_URL + slider.carousel_images.data[1].attributes.url} alt={`Overlay ${index + 1}`} className="w-[90px] h-[91px] ml-[20px] md:w-[175px] md:h-[175px]" />
                                        </div>
                                    )}
                                </div>
                                <div className="flex flex-col items-left text-left md:items-start md:text-left md:flex-1">
                                    <h2 className="text-[20px] mb-4 font-bold text-white md:text-[50px]">{slider.title}</h2>
                                    <p className="text-[16px] text-white md:text-[28px] md:max-w-[800px]">{slider.text}</p>
                                    <button
                                        onClick={() => handleRedirect(index + 1)}
                ...
                {isDesktopOrLaptop && (
                    <>
                        <div className="swiper-button-prev absolute bg-transparent left-[20px] md:left-[200px] md:ml-[200px]">
                            <img src={arrow.src} alt="Previous arrow" className="w-6 h-6 rotate-180 md:w-[48px] md:h-[48px]" />
                        </div>
                        <div className="swiper-button-next absolute right-[20px] md:right-[0px] md:mr-[200px]">
                            <img src={arrow.src} alt="Next arrow" className="w-6 h-6 md:w-[48px] md:h-[48px]" />
                        </div>
                    </>
                )}
